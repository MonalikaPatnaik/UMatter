    ...
    <>
      <div className="mx-auto max-w-2xl px-3 pb-5 pt-3 sm:px-5 sm:pt-4 lg:max-w-7xl lg:px-7 ">
        ...
        <div className="flex flex-row mb-2">
          {blogcontent.tags.map((tag, index) => {
            return (
              <p
                key={index}
                className={`pe-2 text-purple-600 font-semibold	text-lg	`}
              >
                {" "}
                {tag}{" "}
              </p>
        ...
        <img
          ...
          className="object-cover rounded-lg sm:max-h-96 btn- w-9/12 m-6 ms-1 h-full"
        />

        {/* Content --> combination of text , images(optional), title */}
        {blogcontent.content.map((block, index) => {
          return (
            <div className="px-2 pt-5 w-11/12">
              <div
                className={`text-3xl w-5/6 mb-1 ps-2 font-bold ${styles.contentTitle}`}
              >
                {block.title}
              </div>
              {block.image && <img src={block.image} className="py-3" />}

              <div className="text-xl font-medium	ps-2">{block.description}</div>
            </div>
          );
        })}
      </div>
