      ...
      <div className="mx-auto max-w-2xl px-3 pb-5 pt-3 sm:px-5 sm:pt-4 lg:max-w-7xl lg:px-7 ">
        ...
        <div className="flex flex-row mb-2">
          {blogcontent.tags.map((tag, index) => {
            return (
              <p key={index} className={`pe-2 text-purple-600 font-semibold	text-lg	`}>
                {' '}
                {tag}{' '}
              </p>
            );
        ...
        {/* TITLE */}
        <div className={`text-5xl w-5/6 pb-1 font-bold	${styles.title}`}>{blogcontent.title}</div>

        {/* Metatitle */}
        <div className={` text-lg pt-2 ps-1 w-5/6 text-slate-800		 `}>{blogcontent.metatitle}</div>
        ...
        {blogcontent.content.map((block, index) => {
          return (
            <div className="px-2 pt-5 w-11/12">
              ...
              {block.image && <img src={block.image} alt="blog images" className="py-3" />}
